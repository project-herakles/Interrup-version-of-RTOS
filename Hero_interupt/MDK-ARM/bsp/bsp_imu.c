...
void imu_init() // use the 1000 data to let imu converge
{
int i=0;
	while(i<1000)
	{
	  ...
	  off.pit_offset+=imu.pit;
		i++;
	}
	off.pit_offset=(off.pit_offset/1000);
	off.yaw_offset=(off.yaw_offset/1000);
}

void imu_data_filtering(void)
{
	counter++;
	if(counter==1)
	{
	 fil.yaw_buffer1=imu.yaw;
	 fil.pit_buffer2=imu.pit;
	}
	if(counter==2)
	{
		fil.pit_buffer2=imu.pit;
		fil.yaw_buffer2=imu.yaw;
	}	
	if(counter==3)
	{
		fil.pit_buffer3=imu.pit;
		fil.yaw_buffer3=imu.yaw;
	}
	if(counter==4)
	{
		counter--;
		fil.fil_pit=0.5*fil.pit_buffer3+0.3*fil.pit_buffer2+0.2*fil.pit_buffer1;
		fil.fil_yaw=0.5*fil.yaw_buffer3+0.3*fil.yaw_buffer2+0.2*fil.yaw_buffer1;
		fil.pit_buffer1=fil.pit_buffer2;
		fil.pit_buffer2=fil.pit_buffer3;
		fil.pit_buffer3=imu.pit;
		fil.yaw_buffer1=fil.yaw_buffer2;
		fil.yaw_buffer2=fil.yaw_buffer3;
		fil.yaw_buffer3=imu.yaw;
	}
}	
